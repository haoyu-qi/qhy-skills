...
| `.chrome` 右上 | **固定作者徽标** | 每页都使用头像 + 网名 | `<div class="author-badge"><img src="images/avatar.jpg" alt="羡飞鸟妒游鱼头像"><span>羡飞鸟妒游鱼</span></div>` |
...
**正确做法**：chrome 左上是**栏目标签**（稳定、跨页可复用），chrome 右上固定放作者徽标，kicker 是**本页钩子**（短句、有戏剧性），三者互为补充，不互相翻译。

### 固定作者徽标

所有页面右上角默认使用同一个作者徽标，不要改成页码或其他文案：

```html
<div class="author-badge"><img src="images/avatar.jpg" alt="羡飞鸟妒游鱼头像"><span>羡飞鸟妒游鱼</span></div>
```

页码和章节信息放在 `.foot` 或页面内容里，避免和作者署名抢位置。生成项目时必须把 skill 的 `assets/avatar.jpg` 复制到项目的 `images/avatar.jpg`。
