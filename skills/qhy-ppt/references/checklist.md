...
- **chrome 右上角 = 固定作者徽标**:始终使用 `images/avatar.jpg` + `羡飞鸟妒游鱼`
...
右上角固定给作者徽标。页码用 `XX / 总页数` 的格式（比如 `05 / 27`）放在 `.foot`、页面正文元数据或其他不干扰作者位的位置。
...
生成项目时必须把 skill 的 `assets/avatar.jpg` 复制到项目的 `images/avatar.jpg`。每页 `.chrome` 右上角都应使用 `.author-badge`，不要改回页码或其他文案。
